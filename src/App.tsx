    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="mx-auto w-full max-w-[1600px] flex-1 px-4 py-4 sm:px-6">
        ...
        <div className="hidden gap-4 lg:grid lg:grid-cols-[1.4fr_1fr]">
          ...
          {/* Feed */}
          <div className="h-[calc(100vh-200px)] min-h-[500px] overflow-hidden rounded-xl border border-border bg-card">
            <LiveFeed
              requests={requests}
              filter={filter}
              isOnline={isOnline}
        ...
        <div className="lg:hidden">
          ...
          {mobileTab === 'feed' && (
            <div className="h-[calc(100vh-280px)] min-h-[400px] overflow-hidden rounded-xl border border-border bg-card">
              <LiveFeed
                requests={requests}
                filter={filter}
                isOnline={isOnline}
                ...
                return (
                  <div key={s.id} className="rounded-xl border border-border bg-card p-4">
                    <div className="flex items-start justify-between">
                      ...
                      <span className={isFull ? 'text-[10px] font-bold text-alert' : 'text-[10px] font-bold text-success'}>
                        {isFull ? 'FULL' : 'OPEN'}
                      </span>
                    </div>
